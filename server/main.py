...
async def remove_user(req):
    res = await database.remove_user(app.ctx.db, req.args.get("username"), req.args.get("password"))
    if not res:
...
async def edit_profile(req):
    ...
    discord = req.args.get("discord")
    twitter = req.args.get("twitter")
    bio = req.args.get("bio")
