...
*/
	...
	function startTimer() {
		if (!timed_lock) {
			...
			// Setup lock wait
			let wait = 0;
			if (window.sessionStorage.getItem("locktime")) {
				let savedLocktime = parseInt(window.sessionStorage.getItem("locktime"));
				locktime.hours = Math.floor(
					(savedLocktime % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
				);
				locktime.minutes = Math.floor(
					(savedLocktime % (1000 * 60 * 60)) / (1000 * 60)
				);
				locktime.seconds = Math.floor((savedLocktime % (1000 * 60)) / 1000);
			}
			...
			countDownDate.setHours(
				countDownDate.getHours() + (locktime.hours || 0)
			);
			countDownDate.setMinutes(
				countDownDate.getMinutes() + (locktime.minutes || 0)
			);
			countDownDate.setSeconds(
				countDownDate.getSeconds() + (locktime.seconds || 0)
			);

			cdTimer = {};
			let x = setInterval(function () {
				let diff = countDownDate.getTime() - Date.now();
				cdTimer.hours = Math.floor(
					(diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
				);
				cdTimer.minutes = Math.floor(
					(diff % (1000 * 60 * 60)) / (1000 * 60)
				);
				cdTimer.seconds = Math.floor((diff % (1000 * 60)) / 1000);


				// Save time every 5 seconds to avoid unlocking on reload
				if (saveTimeCounter == 2) {
					saveTimeCounter = 0;
					window.sessionStorage.setItem("locktime", diff);
				} else {
					saveTimeCounter++;
				}
				...
				if (diff < 0) {
					...
					window.sessionStorage.removeItem("locktime");
				}
			}, 1000);
		}
	}
	...
	{:then dict}
		<div class="hero bg-primary text-center">
			<div class="hero-body">
				...
				{:catch e}
					<p style="margin-bottom: 0;"> {dict.daily_message_error}</p>
				{/await}
			</div>
		</div>
				...
				{:else}
					...
					{:then tasks}
						...
						{:else}
							...
							<p class="empty-title h4" in:slide={{ delay: 100 }}>
								{dict.no_activities_left}
							</p>
							<p class="empty-subtitle">
								{dict.no_activities_detail}
							</p>
							<button
								class="btn btn-primary"
								on:click={() => {
					...
					{:catch error}
						<div class="empty bg-dark">
							<div class="empty-icon">
								<i class="icon icon-3x icon-cross" />
							</div>
		...
		{:else if tab == 1}
			<div class="empty bg-dark pt-3 pb-6" out:fade={{ duration: 200 }}>
				<div class="empty-icon" in:slide>
					<i class="icon icon-2x icon-shutdown" />
				</div>
				<p
					class="empty-title h3"
					style="padding-bottom: 1rem; font-weight: 500;"
				>
					{(screenlocked
						? dict.screen_locked
						: dict.screen_not_locked
					).capitalize()}
				</p>

				<div
					class="form-group"
					style="display: flex; justify-content: center; width: fit-content; margin-left: auto; margin-right: auto;"
				>
					<button
						...
						on:click={startTimer}
					>
						{screenlocked ? dict.screen_locked : dict.lock_screen}
					</button>
					<label
						class="form-switch"
						style="text-align: center; width: fit-content; margin-left: auto; margin-right: auto;"
					>
						<input
							disabled={timedLockSwitchIsDisabled}
							type="checkbox"
							bind:checked={timed_lock}
						/>
						<i class="form-icon" />
						{dict.timed_lock}
					</label>
				...
				{#if timed_lock}
					...
					{:else}
						<div
							class="form-group"
							style="width: fit-content; margin-left: auto; margin-right: auto; padding-top:
				 1rem; "
						>
							...
							>
								<input
									type="number"
									bind:value={locktime.hours}
									class="form-input input-group-addon"
									min="0"
									placeholder={dict.hours}
								/>
								<input
									type="number"
									bind:value={locktime.minutes}
									class="form-input"
									min="0"
									max="59"
									placeholder={dict.minutes}
								/>
								<input
									type="number"
									bind:value={locktime.seconds}
									class="form-input input-group-addon"
									min="0"
									max="59"
									placeholder={dict.seconds}
								/>
							</div>
						</div>
					{/if}
		...
		{:else if tab == 2}
			<div class="empty bg-dark pt-3 pb-6" out:fade={{ duration: 200 }}>
				<div class="empty-icon" in:slide>
					<i class="icon icon-2x icon-bookmark" />
				</div>
				<p class="empty-title h4" in:slide={{ delay: 100 }}>
					{dict.subjects}
				</p>
				<div class="form-group mt-2">
					<label class="form-label" for="create-subj"
						>{dict.create_subject}</label
					>
					<div class="input-group">
						<input
							bind:value={create_subject_name}
							style="border: #5755d9 3px solid!important; border-right: transparent;"
							class="form-input"
							type="text"
							id="create-subj"
							placeholder={dict.math}
						/>
						...
						{:else}
							<button
								class="btn btn-primary input-group-btn"
								disabled>{dict.add}</button
							>
						{/if}
					</div>
					<p class="form-input-hint">
						{!create_subject_name ? dict.field_empty : ""}
					</p>
				</div>
		...
		{:else if tab == 6}
			<div class="empty bg-dark pt-3 pb-6" out:fade={{ duration: 200 }}>
				...
				<!-- Store documents in an ordered way -->
				<p>{dict.documents_hint}</p>
			</div>
		{:else if tab == 4}
			<div class="empty bg-dark pt-3 pb-6" out:fade={{ duration: 200 }}>
				<div class="empty-icon" in:slide>
					<i class="icon icon-2x icon-edit" />
				</div>
				<p class="empty-title h4" in:slide={{ delay: 100 }}>
					{dict.settings}
				</p>
				...
				<button class="btn btn-primary mt-2" on:click={saveSettings}
					>{dict.save}</button
				>
			</div>
		{/if}
	{:catch error}
		<svg
			...
			/><path
				...
				fill="#284676"
			/><path
				d="M0 69L11.5 67.2C23 65.3 46 61.7 69 57.5C92 53.3 115 48.7 138.2 48.2C161.3 47.7 184.7 51.3 207.8 55.7C231 60 254 65 277 64C300 63 323 56 346 55.7C369 55.3 392 61.7 415.2 64.2C438.3 66.7 461.7 65.3 484.8 63C508 60.7 531 57.3 554 54C577 50.7 600 47.3 623 48.5C646 49.7 669 55.3 692.2 59.7C715.3 64 738.7 67 761.8 67.5C785 68 808 66 831 62.2C854 58.3 877 52.7 888.5 49.8L900 47L900 0L888.5 0C877 0 854 0 831 0C808 0 785 0 761.8 0C738.7 0 715.3 0 692.2 0C669 0 646 0 623 0C600 0 577 0 554 0C531 0 508 0 484.8 0C461.7 0 438.3 0 415.2 0C392 0 369 0 346 0C323 0 300 0 277 0C254 0 231 0 207.8 0C184.7 0 161.3 0 138.2 0C115 0 92 0 69 0C46 0 23 0 11.5 0L0 0Z"
				fill="#2e51aa"
			/><path
				d="M0 28L11.5 29.2C23 30.3 46 32.7 69 34.7C92 36.7 115 38.3 138.2 37.2C161.3 36 184.7 32 207.8 31.7C231 31.3 254 34.7 277 34.8C300 35 323 32 346 32.7C369 33.3 392 37.7 415.2 39.3C438.3 41 461.7 40 484.8 37.5C508 35 531 31 554 31.3C577 31.7 600 36.3 623 38.8C646 41.3 669 41.7 692.2 41.2C715.3 40.7 738.7 39.3 761.8 37.7C785 36 808 34 831 33.5C854 33 877 34 888.5 34.5L900 35L900 0L888.5 0C877 0 854 0 831 0C808 0 785 0 761.8 0C738.7 0 715.3 0 692.2 0C669 0 646 0 623 0C600 0 577 0 554 0C531 0 508 0 484.8 0C461.7 0 438.3 0 415.2 0C392 0 369 0 346 0C323 0 300 0 277 0C254 0 231 0 207.8 0C184.7 0 161.3 0 138.2 0C115 0 92 0 69 0C46 0 23 0 11.5 0L0 0Z"
				fill="#5755d9"
			/>
		</svg>
		<!-- Reduce margin ^ -->
		<div class="empty bg-dark" style="height: 100%;">
			...
			<p>{error.message}</p>
			<p class="empty-subtitle">
				Try searching for this issue on our GitHub repo. If you can not
				find a GitHub issue around this, please create one.
			</p>
		</div>
	{/await}
</main>
